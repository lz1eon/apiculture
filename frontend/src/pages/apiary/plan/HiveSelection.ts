...
import { Hive, HiveObjectKey } from "../../../models";
import { Highlight } from './ApiaryPlan';
...
export class HiveSelection {
  ...
  highlight(highlight: Highlight) {
    for (let color in highlight.colors) {
      this.svgElement
        .selectAll(`g.hive[hive-${highlight.prop}="${color}"]`)
        .attr('fill', highlight.colors[color])
    }
  }

  clearHighlight() {
    this.svgElement.selectAll('g.hive').attr('fill', this.DEFAULT_COLOR);
  }
